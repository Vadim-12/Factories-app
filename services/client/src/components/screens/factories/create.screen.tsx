...
import { IFactoryWithOctets } from '@/types/factory';
import { useAppDispatch } from '@/hooks/store';
import FactoryFormatter from '@/utils/factoryFormatter';
import validationSchema from '@/utils/validationSchema';
import { createFactory } from '@/store/slices/factorySlice';
import { redirect } from 'next/navigation';

const CreateFactoryScreen: React.FC = () => {
	...
	const [hostOctet1, setHostOctet1] = useState<string>('');
	const [hostOctet2, setHostOctet2] = useState<string>('');
	const [hostOctet3, setHostOctet3] = useState<string>('');
	const [hostOctet4, setHostOctet4] = useState<string>('');
	...
	const initialValues: IFactoryWithOctets = {
		index: '',
		name: '',
		hostOctet1: '',
		hostOctet2: '',
	...
	return (
		<div className='container containerEditFactoryPage'>
			...
			<Formik
				...
				validateOnBlur
				onSubmit={async (newFactory: IFactoryWithOctets) => {
					const formattedNewFactory =
						FactoryFormatter.factoryWithOctetsToFactoryWithHost(newFactory);
					await dispatch(createFactory(formattedNewFactory));
					redirect('/factories');
				}}
